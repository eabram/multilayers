...
class ML():
    def __init__(self,n_list=[],d_list=[],labda=800.0e-9,FWHM=50.0e-9,fill_value=np.nan,direct_n='/media/sf_Documents/Calculations/Multilayer_calculations/Refractive Index/',k_off=[],pos_sub='Default',n_adjust={},backscatter=False):
        self.labda0 = labda
        self.fill_value = fill_value
        self.n_dict = self.get_n(direct=direct_n,k_off=k_off)
        self.n = self.get_n_value(n_list,n_adjust)
        self.mat = n_list
        self.d = d_list
        self.FWHM = FWHM
    ...
    def get_n_value(self,mat: list,n_adjust: dict) -> list:
        #nmat = {'Au': 0.112759+4.712772j,
        #        'BK7' : 1.5108 + 1j*9.2656e-9,
        #        'Air' : 1.00027505,
        #        'Al' :  2.7673 + 1j*8.3543,
        #        'Al2O3' : 1.7601}
        # Guido '/media/sf_Documents/Calculations/Dielectric function/Data/n&k_ellipsometry_Au_250nm.csv'
        ret = []
        for m in mat:
            if m in n_adjust:
                ret.append(n_adjust[m])
                #print('Value: '+str(n_adjust[m]))
                #print(ret[-1](self.labda0))
            else:
                ret.append(self.n_dict[m])

        return ret
    def get_n_val(self,labda='Default',n='Default'):
        if n=='Default':
            n = self.n
    ...
    def get_Mij(self,n,d,j,labda='Default',pol='p',ang=0.0):
        ...
        #th1=np.arcsin((np.sin(ang)*np.real(n0))/np.real(n1))
        #th2=np.arcsin((np.sin(ang)*np.real(n0))/np.real(n2))
        sinth1=(np.sin(ang)*np.real(n0))/realn1
        sinth2=(np.sin(ang)*np.real(n0))/realn2
        n1par=(np.sin(ang)*np.real(n0))
        #n1par = np.real(n1)*sinth1
        n2par=(np.sin(ang)*np.real(n0))
        #n2par = np.real(n2)*sinth2
        ...
        #k1perp = (realn1*imagn1)/n1perp
        #k2perp = (realn2*imagn2)/n2perp
        ...
        #Mjm1j=np.matrix([[1.0/t, r/t],[r/t, ((r**2)/(t))+((t*beta*alpha)/1.0)]])
        ...
        if j!=len(n)-1:
            ...
            if j==self.pos_sub and self.backscatter==False:
                ...
                #Mj = np.array([[np.exp(-1j*k0*n2perp*dj)*np.exp(k0*k2perp*dj),0],[0,0]]) # No back-reflection
            else:
                Mj = np.array([[np.exp(-1j*kd*dj),0],[0,np.exp(1j*kd*dj)]])
                Mjz = lambda z: np.array([[np.exp(-1j*kd*z),0],[0,np.exp(1j*kd*z)]])
                #Mj = np.array([[np.exp(-1j*k0*n2perp*dj)*np.exp(k0*k2perp*dj),0],[0,np.exp(1j*k0*n2perp*dj)*np.exp(-k0*k2perp*dj)]])
    ...
    def run(self,n='Default',d='Default',labda0='Default',FWHM='Default',option='numerical',steps=401,ang=0.0,pol='p',a=3):
        ...
        if option=='pulse':
            ...
            #val = np.array([np.array(self.calc_multilayer(labda,n=n,d=d,ang=ang,pol=pol))*self.pulse(labda) for labda in labda_vec])
            ...
            #    abs_func_val.append(lambda z: self.get_absorption(z,Mz_val[i],T_val[i],d,n,labda_vec[i]))
            #i=0
            #print(self.get_absorption(10.0e-9,Mz_val[i],T_val[i],d,n,labda_vec[i])))
            #i=100
            #print(self.get_absorption(10.0e-9,Mz_val[i],T_val[i],d,n,labda_vec[i])))
            ...
            #abs_func = lambda z: self.get_multiple_abs_wf(z,abs_func_val)#*Dlabda
            Rerr = np.nan
            Terr = np.nan
            rerr = np.nan
    ...
    def get_n(self,direct='/media/sf_Documents/Calculations/Multilayer_calculations/Refractive Index/',sel='model',k_off=[]):
        out = {}
        fn_all = []
